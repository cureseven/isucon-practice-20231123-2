...
func tenantsBillingHandler(c echo.Context) error {
	...
	// 最初にすべての競技会を取得します。
	allCompetitions := []CompetitionRow{}
	if err := adminDB.SelectContext(ctx, &allCompetitions, "SELECT * FROM competition WHERE finished_at IS NOT NULL"); err != nil {
		return fmt.Errorf("failed to Select all competitions: %w", err)
	}

	// 競技会をテナントIDごとにグループ化します。
	competitionsByTenant := make(map[int64][]CompetitionRow)
	for _, comp := range allCompetitions {
		competitionsByTenant[comp.TenantID] = append(competitionsByTenant[comp.TenantID], comp)
	}
	...
	for _, t := range ts {
		...
		err := func(t TenantRow) error {
			...
			for _, comp := range competitionsByTenant[t.ID] {
...
func competitionFinishHandler(c echo.Context) error {
	...
	_, err = retrieveCompetition(ctx, adminDB, v.tenantID, id)
	if err != nil {
		// 存在しない大会
		if errors.Is(err, sql.ErrNoRows) {
...
func competitionScoreHandler(c echo.Context) error {
	...
	comp, err := retrieveCompetition(ctx, adminDB, v.tenantID, competitionID)
	if err != nil {
		// 存在しない大会
		if errors.Is(err, sql.ErrNoRows) {
			return echo.NewHTTPError(http.StatusNotFound, "competition not found")
	...
	run := func() error {
		...
		if len(playerScoreRows) > 0 {
			...
			if _, err := tx.NamedExecContext(
				ctx,
				"INSERT INTO player_score (id ,tenant_id, player_id,row_num, competition_id, score, created_at, updated_at) VALUES (:id ,:tenant_id, :player_id, :row_num,:competition_id, :score, :created_at, :updated_at)",
				playerScoreRows,
			); err != nil {
				return fmt.Errorf(
...
func competitionRankingHandler(c echo.Context) error {
	...
	// 大会の存在確認
	competition, err := retrieveCompetition(ctx, adminDB, v.tenantID, competitionID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return echo.NewHTTPError(http.StatusNotFound, "competition not found")
		}
	...
	for _, ps := range pss {
		...
		p, err := retrievePlayer(ctx, adminDB, ps.PlayerID)
		if err != nil {
			return fmt.Errorf("error retrievePlayer: %w", err)
		}
